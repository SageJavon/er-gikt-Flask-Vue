...
for epoch in range(params['epochs']):
    ...
    for data in data_loader:
        ...
        train_step += 1
        print(
            f'train_step: {train_step}, loss: {loss.item():.4f}, acc: {acc.item():.4f}, auc: {auc:.4f}')
    time1 = time.time()
    ...
    # 输出至终端
    print(
        LOG_B + f'epoch: {epoch}, loss: {loss_aver:.4f}, acc: {acc_aver:.4f}, auc: {auc_aver: .4f}' + LOG_END)
    print(
        LOG_B + f'time: {run_time:.2f}s, average batch time: {(run_time / train_step):.2f}s' + LOG_END)
    # 保存输出至本地文件
    output_file.write(
        f'epoch: {epoch}, loss: {loss_aver:.4f}, acc: {acc_aver:.4f}, auc: {auc_aver: 4f}\n')
    output_file.write(
        f'time: {run_time:.2f}s, average batch time: {(run_time / train_step):.2f}s\n')
output_file.close()
torch.save(model, f=f'model/result.pt')
np.savetxt(f'chart_data/chart.txt', y_label)
