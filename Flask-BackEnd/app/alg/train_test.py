...
qs_table = torch.tensor(sparse.load_npz(
    'data/qs_table.npz').toarray(), dtype=torch.int64, device=DEVICE)  # [num_q, num_c]
...
y_label_all = np.zeros([3, params['epochs'] * params['k_fold']+1])  # 全部精度值
...
for epoch in range(params['epochs']):
    ...
    for fold, (train_indices, test_indices) in enumerate(k_fold.split(dataset)):
        ...
        if DEVICE.type == 'cpu':  # Cpu(本机)
            train_loader = DataLoader(
                train_set, batch_size=batch_size)  # 训练数据加载器
            test_loader = DataLoader(
                test_set, batch_size=batch_size)  # 测试数据加载器
        else:  # Gpu(服务器)
        ...
        print(
            LOG_B + f'training: loss: {train_loss:.4f}, acc: {train_acc:.4f}, auc: {train_auc: .4f}' + LOG_END)
        print(
            LOG_B + f'testing: loss: {test_loss:.4f}, acc: {test_acc:.4f}, auc: {test_auc: .4f}' + LOG_END)
        print(
            LOG_B + f'time: {run_time:.2f}s, average batch time: {(run_time / test_step):.2f}s' + LOG_END)
        # 保存输出至本地文件
        ...
        output_file.write(
            f'time: {run_time:.2f}s, average batch time: {(run_time / test_step):.2f}s\n')
    ...
    print('>>>>>>>>>>>>>>>>>>' + LOG_Y +
          f"epoch: {epoch_total - params['k_fold']} - {epoch_total}" + LOG_END + '<<<<<<<<<<<<<<<<<<')
    print(
        LOG_G + f'training: loss: {train_loss_aver:.4f}, acc: {train_acc_aver:.4f}, auc: {train_auc_aver: .4f}' + LOG_END)
    print(
        LOG_G + f'testing: loss: {test_loss_aver:.4f}, acc: {test_acc_aver:.4f}, auc: {test_auc_aver: .4f}' + LOG_END)
    output_file.write(
        f"epoch {epoch_total - params['k_fold']} - {epoch_total} | ")
    output_file.write(
        f'training: loss: {train_loss_aver:.4f}, acc: {train_acc_aver:.4f}, auc: {train_auc_aver: .4f} | ')
    output_file.write(
        f'testing: loss: {test_loss_aver:.4f}, acc: {test_acc_aver:.4f}, auc: {test_auc_aver: .4f}\n')
    y_label_aver[0][epoch], y_label_aver[1][epoch], y_label_aver[2][epoch] = test_loss_aver, test_acc_aver, test_auc_aver

output_file.close()
torch.save(model, f=f'model-200/result.pt')
np.savetxt(f'chart_data_200/result_all.txt', y_label_all)
np.savetxt(f'chart_data_200/result_aver.txt', y_label_aver)
